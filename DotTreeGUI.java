...
public class DotTreeGUI extends DrawingGUI {
	...
	//test the axis and make sure they go in the same 
	public void test2(){
		found= null;
		tree = new PointQuadtree<Dot>(new Dot(300,400), 0,0,800,600);
		tree.insert(new Dot((int) tree.getX1(), (int) tree.getY1())); // B
		tree.insert(new Dot((int) tree.getPoint().getX(),(int) tree.getPoint().getY())); // C

		//visually check if the lines are being added on the raw quadtree
	...
	//do some random clicking without testings
	//second just write the test 2 do additional so that find and insert works
	//size and all points are working.
	/**
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		// TODO: YOUR CODE HERE
		g.fillOval((int) tree.getPoint().getX(), (int) tree.getPoint().getY(), diameter, diameter);
		g.drawLine((int) tree.getPoint().getX()+ dotRadius, tree.getY1()+dotRadius, (int) tree.getPoint().getX()+ dotRadius , tree.getY2()+dotRadius);
		g.drawLine((int) tree.getX1()+dotRadius, (int) tree.getPoint().getY()+ dotRadius, (int) tree.getX2()+dotRadius, (int) tree.getPoint().getY()+dotRadius);
